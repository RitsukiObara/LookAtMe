...
HRESULT CTime::Init(void)
{
	...
	// テクスチャの読み込み処理
	nTexIdx = CManager::GetTexture()->Regist(TIME_TEXTURE);
	...
	if (m_pDot != nullptr &&
		CManager::GetTexture() != nullptr)
	{ // タイムの点が NULL じゃないかつ、テクスチャが NULL じゃない場合

		// テクスチャの割り当て処理
		m_pDot->BindTexture(CManager::GetTexture()->Regist(TIMEDOT_TEXTURE));
	}
	else
	{ // 上記以外
...
void CTime::SetNumber(void)
{
	...
	// 10進数への変換処理
	DecimalCalculation(MAX_TIME_DIGIT, m_TotalTime, &aNum[0]);
