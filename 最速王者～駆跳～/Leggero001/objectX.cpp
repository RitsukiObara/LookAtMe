...
{
	// オブジェクトモデル
	"data/MODEL/EnergyItem.x",			// エネルギーアイテム
	"data/MODEL/Fraction.x",			// 破片
	"data/MODEL/GoalUI.x",				// ゴールUI
	"data/MODEL/Ripple.x",				// 着地の波紋
	"data/MODEL/ShotEnemy.x",			// 弾撃ち敵
	"data/MODEL/RankSS.x",				// SSランク表示
	"data/MODEL/RankS.x",				// Sランク表示
	"data/MODEL/RankA.x",				// Aランク表示
	"data/MODEL/RankB.x",				// Bランク表示
	"data/MODEL/RankC.x",				// Cランク表示
	"data/MODEL/ItemGenerator.x",		// アイテム生成器
	"data/MODEL/ArrowPole.x",			// 矢印看板の柱
	"data/MODEL/ArrowSign.x",			// 矢印看板

	// ブロックモデル
	"data/MODEL/DirtBlock.x",			// 地面ブロック
	"data/MODEL/RockBlock.x",			// 岩ブロック
	"data/MODEL/IceBlock.x",			// 氷ブロック
	"data/MODEL/MagmaBlock.x",			// マグマブロック

	// 障害物モデル
	"data/MODEL/Hydrant.x",				// 消火栓
	"data/MODEL/SignPole.x",			// 標識
	"data/MODEL/Drum.x",				// ドラム缶
	"data/MODEL/WoodBox.x",				// 木箱
	"data/MODEL/StreetLamp.x",			// 街灯(光源)
	"data/MODEL/StreetPole.x",			// 街灯(支柱)
	...
	// エディットモデル
	"data/MODEL/collision000.x",		// 当たり判定
};
	...
	{ // モデルに使用するモデルの最大数分繰り返す
		...
		{ // マテリアルの数分繰り返す
			...
			{ // テクスチャファイルが存在する場合
				...
				// テクスチャの読み込み処理
				m_apModel[nCntModel].m_nTexIdx[nCntMat] = CManager::GetTexture()->Regist(pMat[nCntMat].pTextureFilename);
			}
		}
	}
