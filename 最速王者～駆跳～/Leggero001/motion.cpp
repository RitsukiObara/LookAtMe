...
void CMotion::Set(int nType)
{
	...
	case TYPE_PLAYER:		// プレイヤー
		...
		if (m_nType == CPlayer::MOTIONTYPE_WALLKICK)
		{ // 壁キックの場合
			...
			for (int nCntModel = 0; nCntModel < m_nNumModel; nCntModel++)
			{
				...
				// パーツの位置・向きを設定
				rot.x = rot.x + m_aInfo[m_nType].aKeyInfo[m_nKey].aKey[nCntModel].fRotX;
				rot.y = rot.y + m_aInfo[m_nType].aKeyInfo[m_nKey].aKey[nCntModel].fRotY;
				rot.z = rot.z + m_aInfo[m_nType].aKeyInfo[m_nKey].aKey[nCntModel].fRotZ;
...
void CMotion::Update(void)
{
	...
	{ // 終了していない場合
		...
		{ // 全モデル(パーツ)の更新
			...
			// 向きの差分の正規化
			RotNormalize(&rotDiff.x);
			RotNormalize(&rotDiff.y);
			RotNormalize(&rotDiff.z);
				...
				{ // ループしない場合
					...
					case TYPE_PLAYER:		// プレイヤー
						...
						if (m_nType == CPlayer::MOTIONTYPE_LANDING ||
							m_nType == CPlayer::MOTIONTYPE_START ||
							m_nType == CPlayer::MOTIONTYPE_ATTACK)
						{ // 着地または、スタート前または、攻撃モーションの場合

							// 通常モーションに設定する
							Set(CPlayer::MOTIONTYPE_NEUTRAL);
						}
						else if (m_nType == CPlayer::MOTIONTYPE_WALLSTEPIN)
						{ // 壁踏み込みモーションの場合

							// 壁キックモーションに設定する
							Set(CPlayer::MOTIONTYPE_WALLKICK);
						}
						else
						{ // 上記以外
					...
					case TYPE_KICKENEMY:	// 障害物キック敵

						break;

					case TYPE_SHOTENEMY:	// 射撃敵
		...
		// デバッグ
		CManager::GetDebugProc()->Print("モーション中\n");
	}
	else
	{ // 上記以外

		// デバッグ
		CManager::GetDebugProc()->Print("モーション終了\n");
	}
}
