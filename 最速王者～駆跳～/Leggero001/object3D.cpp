...
void CObject3D::SetVertex(void)
{
	...
	// 法線の計算(正規化)
	NormalizeVector(nor[0], pVtx[2].pos, pVtx[1].pos, pVtx[0].pos);
	NormalizeVector(nor[1], pVtx[0].pos, pVtx[3].pos, pVtx[1].pos);
	NormalizeVector(nor[2], pVtx[3].pos, pVtx[0].pos, pVtx[2].pos);
	NormalizeVector(nor[3], pVtx[1].pos, pVtx[2].pos, pVtx[3].pos);
...
float CObject3D::ElevationCollision(const D3DXVECTOR3 pos)
{
	...
	if (LineOuterProductXZ(pVtx[0].pos, pVtx[1].pos, pos) >= 0 &&
		LineOuterProductXZ(pVtx[1].pos, pVtx[2].pos, pos) >= 0 &&
		LineOuterProductXZ(pVtx[2].pos, pVtx[0].pos, pos) >= 0)
	{ // 真ん中の境界線より右側に居た場合

		// 法線の計算(正規化)
		NormalizeVector(nor, pVtx[2].pos, pVtx[1].pos, pVtx[0].pos);
	...
	else if(LineOuterProductXZ(pVtx[1].pos, pVtx[3].pos, pos) >= 0 &&
		LineOuterProductXZ(pVtx[2].pos, pVtx[1].pos, pos) >= 0 &&
		LineOuterProductXZ(pVtx[3].pos, pVtx[2].pos, pos) >= 0)
	{ // 真ん中の境界線より左側に居た場合

		// 法線の計算(正規化)
		NormalizeVector(nor, pVtx[1].pos, pVtx[2].pos, pVtx[3].pos);
