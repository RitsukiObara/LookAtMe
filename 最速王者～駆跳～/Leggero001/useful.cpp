...
// 重力処理
//======================
void Gravity(float *fGravity, D3DXVECTOR3& pPos, const float fAdd)
{
	// 重力を加算する
	*fGravity -= fAdd;
...
// ホーミング処理
//======================
void RotCorrect(const float fDest, float *pRot, const float fCorrect)
{
	// ローカル変数宣言
...
// 値の補正処理
//======================
void Correct(const float fDest, float *pValue, const float fCorrect)
{
	// ローカル変数宣言
	float fDiff;			// 値の差分
...
// 円の当たり判定(XY平面)
//======================
bool CircleCollisionXY(const D3DXVECTOR3& pos0, const D3DXVECTOR3& pos1, const float fRadius0, const float fRadius1)
{
	// ローカル変数宣言
	float fLength;
...
// 円の当たり判定(XZ平面)
//======================
bool CircleCollisionXZ(const D3DXVECTOR3& pos0, const D3DXVECTOR3& pos1, const float fRadius0, const float fRadius1)
{
	// ローカル変数宣言
	float fLength;
...
// 外積の左右判断処理
//======================
float LineOuterProductXZ(D3DXVECTOR3 posLeft, D3DXVECTOR3 posRight, D3DXVECTOR3 pos)
{
	// 変数を宣言
...
// 外積の左右判断処理(XY平面)
//======================
float LineOuterProductXY(D3DXVECTOR3 posUp, D3DXVECTOR3 posDown, D3DXVECTOR3 pos)
{
	// 変数を宣言
	D3DXVECTOR3 vecLine;	// 境界線ベクトル
	D3DXVECTOR3 vecToPos;	// 左端と位置のベクトル
...
// 法線の計算(正規化)
//======================
void NormalizeVector(D3DXVECTOR3& nor, const D3DXVECTOR3& posRight, const D3DXVECTOR3& posLeft,const D3DXVECTOR3& pos)
{
	// ローカル変数宣言
...
// 内積の判定処理
//======================
float InnerProduct(const D3DXVECTOR3 VecMove, const D3DXVECTOR3 VecNor)
{
	// ローカル変数宣言
	float fDot = 0.0f;			// 内積
...
// 放物線の計算
//======================
float Parabola(const float fPosX, const float fShiftX, float fAddHeight, const float fShiftY, float fMagni)
{
	// ローカル変数宣言
	float fPosY = 0.0f;

	// 放物線の計算
	fPosY = fAddHeight * (((fPosX * fMagni) - fShiftX) * ((fPosX * fMagni) - fShiftX)) + fShiftY;
...
// 外積の当たり判定処理(XZ軸版)
//======================
bool CollisionOuterProductXZ(D3DXVECTOR3& Targetpos, D3DXVECTOR3& TargetposOld, const D3DXVECTOR3 posRight, const D3DXVECTOR3 posLeft)
{
	// ローカル変数宣言
...
// 外積の当たり判定処理(XY軸版)
//======================
bool CollisionOuterProductXY(D3DXVECTOR3& Targetpos, D3DXVECTOR3& TargetposOld, const D3DXVECTOR3 posRight, const D3DXVECTOR3 posLeft)
{
	// ローカル変数宣言
	D3DXVECTOR3 vecMove, vecLine, vecTopos, posCross;	// 外積の変数
	float fRate;										// 割合の変数
...
// 矩形の当たり判定
//======================
bool RectangleCollisionXY(const D3DXVECTOR3& pos0, const D3DXVECTOR3& pos1, const D3DXVECTOR3& VtxMax0, const D3DXVECTOR3& VtxMax1, const D3DXVECTOR3& VtxMin0, const D3DXVECTOR3& VtxMin1)
{
...
// 円柱の当たり判定
//======================
void CylinderCollision(D3DXVECTOR3* TargetPos, const D3DXVECTOR3& ObjectPos, const float fObjectRadius)
{
	// ローカル変数宣言
...
// 10進数への変換処理
//======================
void DecimalCalculation(const int nDigit, const int nValue, int* aAnswer)
{
...
// 均等な数値の補正
//======================
void FrameCorrect(const float fDest, float *fTarget, const float fCorrect)
{
	if (*fTarget > fDest)
	{ // サイズが目標超過だった場合
