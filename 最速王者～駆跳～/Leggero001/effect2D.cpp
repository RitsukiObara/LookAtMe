...
void CEffect2D::SetData(const D3DXVECTOR3& pos, const D3DXVECTOR3& move, const int nLife, const float fRadius, const TYPE type)
{
	...
	{ // 種類が星関係の場合
		...
		// テクスチャの読み込み処理
		BindTexture(CManager::GetTexture()->Regist(STAR_EFFECT_TEXTURE));
	}
	...
	{ // 種類が炎・ブーストの場合
		...
		// テクスチャの読み込み処理
		BindTexture(CManager::GetTexture()->Regist(BOOST_EFFECT_TEXTURE));
	}
	else
	{ // 上記以外

		// テクスチャの読み込み処理
		BindTexture(CManager::GetTexture()->Regist(NORMAL_EFFECT_TEXTURE));
	}
}
