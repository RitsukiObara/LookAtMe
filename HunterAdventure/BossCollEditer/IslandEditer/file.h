...
private:		// 自分のみアクセスできる
	...
	HRESULT SaveCoin(void);			// コインのセーブ処理
	HRESULT SaveGoldBone(void);		// 金の骨のセーブ処理
	HRESULT SaveTree(void);			// 木のセーブ処理
	HRESULT SaveRock(void);			// 岩のセーブ処理
	HRESULT SaveBlock(void);		// ブロックのセーブ処理
	HRESULT SaveBangFlower(void);	// 爆弾花のセーブ処理
	HRESULT SaveWall(void);			// 壁のセーブ処理
	HRESULT SaveBossColl(void);		// ボスの当たり判定のセーブ処理
