...
CHierarchy::CHierarchy()
{
	...
	m_nParentIdx = NONE_PARENT;							// 親モデルのインデックス
}
...
// 親モデルのインデックスの設定処理
//========================
void CHierarchy::SetParentIdx(const int nIdx)
{
	// 親モデルのインデックスを設定する
	m_nParentIdx = nIdx;
}
...
// 親モデルのインデックスの取得処理
//========================
int CHierarchy::GetParentIdx(void) const
{
	// 親モデルのインデックスを取得する
	return m_nParentIdx;
}
...
// オーバーロードコンストラクタ
//========================
CCharacter::CCharacter(CObject::TYPE type, PRIORITY priority) : CObject(type, DIM_3D, priority)
{
	// 全ての値をクリアする
	m_pos = D3DXVECTOR3(0.0f, 0.0f, 0.0f);				// 位置
	m_posOld = D3DXVECTOR3(0.0f, 0.0f, 0.0f);			// 前回の位置
