	...
	{ // ボスの当たり判定が読み込めた場合
		...
		for (int nCntPart = 0; nCntPart < m_BossCollFile.nNumData; nCntPart++)
		{
			...
			for (int nCntColl = 0; nCntColl < m_BossCollFile.aFile[nCntPart].nNum; nCntColl++)
			{
				...
				pColl[nCntPart]->SetWeakness(m_BossCollFile.aFile[nCntPart].aBase[nCntColl].bWeakness, nCntColl);
			}
		}
	}
	else
...
// ボスの当たり判定のセーブ処理
//===========================================
HRESULT CFile::SaveBossColl(void)
{
	CBoss* pBoss = CGame::GetEdit()->GetBoss();
	CBossCollision* pBossColl = nullptr;

	// ポインタを宣言
	FILE* pFile;				// ファイルポインタ

	// ファイルを読み込み形式で開く
	pFile = fopen(BOSSCOLL_TXT, "w");
	...
	{ // ファイルが開けた場合
		...
		{ // 空白じゃない場合
			...
			// モデルの総数を書き込む
			fprintf(pFile, "NUM_MODEL = ");
			fprintf(pFile, "%d\n\n", pBoss->GetNumModel());

			for (int nCnt = 0; nCnt < pBoss->GetNumModel(); nCnt++)
			{
				// 当たり判定の情報を取得
				pBossColl = CGame::GetEdit()->GetColl(nCnt);

				if (pBossColl != nullptr)
				{ // 当たり判定が NULL じゃない場合

					// セーブ開始
					fprintf(pFile, "SET_COLL\n");

					// 当たり判定の総数を書き込む
					fprintf(pFile, "\tNUM_COLL = ");
					fprintf(pFile, "%d\n\n", pBossColl->GetNumColl());

					for (int nCntColl = 0; nCntColl < pBossColl->GetNumColl(); nCntColl++)
					{
						// 位置を書き込む
						fprintf(pFile, "\tPOS = ");
						fprintf(pFile, "%.1f %.1f %.1f\n", pBossColl->GetCollOffset(nCntColl).x, pBossColl->GetCollOffset(nCntColl).y, pBossColl->GetCollOffset(nCntColl).z);

						// 半径を書き込む
						fprintf(pFile, "\tRADIUS = ");
						fprintf(pFile, "%.1f\n", pBossColl->GetRadius(nCntColl));

						// 弱点状況を書き込む
						fprintf(pFile, "\tWEAK = ");

						if (pBossColl->GetWeakness(nCntColl) == true)
						{ // 弱点状況が true の場合
