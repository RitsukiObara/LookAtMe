...
public:
	...
	void Draw(CBoss* pBoss, const int nIdx);		// 描画処理
	...
	void SetWeakness(const bool bWeak, const int nIdx);					// 弱点状況の設定処理
	bool GetWeakness(const int nIdx) const;								// 弱点状況の取得処理
	void SetNumColl(const int nNum);				// 当たり判定の総数の設定処理
	int GetNumColl(void) const;						// 当たり判定の総数の取得処理
	void SetModel(const int nIdx);					// モデルの設定処理
	void DeleteModel(const int nIdx);				// モデルの消去処理
