...
// オーバーロードコンストラクタ
//=========================================
CDonut2D::CDonut2D(CObject::TYPE type, PRIORITY priority) : CObject(type, DIM_2D, priority)
{
	// 全ての値をクリアする
...
// 情報の設定処理
//===========================================
void CDonut2D::SetData(const D3DXVECTOR3& pos, const float fSize, const float fDist, const int nDivi)
{
	// 情報を設定する
	m_pos = pos;				// 位置
	m_posOld = pos;				// 前回の位置
	m_fSize = fSize;			// サイズ
	m_fDistance = fDist;		// 距離
	m_nDivi = nDivi;			// 分割数
	m_nTexIdx = NONE_TEXIDX;	// テクスチャ
	SetNumVtx();				// 総頂点数
}
...
// 生成処理
//===========================================
CDonut2D* CDonut2D::Create(const D3DXVECTOR3& pos, const float fSize, const float fDist, const int nDivi, const CObject::TYPE type, const PRIORITY priority)
{
	// ローカルオブジェクトを生成
	...
	{ // オブジェクトが NULL の場合
		...
		// オブジェクトを生成
		pDonut = new CDonut2D(type, priority);
	}
	else
	{ // オブジェクトが NULL じゃない場合
	...
	{ // オブジェクトが NULL じゃない場合
		...
		// 情報の設定処理
		pDonut->SetData(pos, fSize, fDist, nDivi);
